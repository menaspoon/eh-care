...
<form  action="{{ url("store.approve.pharmacy.service") }}" method="POST" class="companies container form-contract">
  <br>
  <div class="row">
    @csrf
    <div class="col-md-6">
      <h3> مراجعـــة الطــــــوارئ</h3>
      <br><br>
    </div> <!-- col-md-3 -->
    <div class="col-md-6">
      @if (Session::has("created"))
        ...
        <table class="table" id="invoice_detales">
          ...
          <tbody>
            <tr class="invoice_parent_row">
              ...
              <td>
                <select id="SelExample" name="service[]" class="get_service_price  form-control">
                  ...
                  @foreach ($pharmacy as $item)
                    <option data-small_price="{{ $item->small_price }}" 
                            data-large_price="{{ $item->large_price }}" 
                            data-id="{{ $item->id }}" 
                            value="{{ $item->id }}">
                            {{ $item->name }}
                      </option>
              ...
              <td>
                <select id="SelExample" name="service[]" class="get_unit  form-control">
                  <option value="">الوحدة</option>
                  <option class="small_price" data-price="" value="">وحدة كبري</option>
                  <option class="large_price" data-price=""> وحدة صغري</option>
                </select>
              </td>
              <td>
...
$(document).on("change", ".get_service_price", function () {
    var small_price =  $(this).find(':selected').data('small_price');
    var large_price =  $(this).find(':selected').data('large_price');
    $(this).parent().next().children().find('.small_price').val(small_price)
    $(this).parent().next().children().find('.large_price').val(large_price)
});

$(document).on("change", ".get_unit", function () {
