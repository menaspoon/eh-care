...
class PhysicalTherapyController extends Controller {
  ...
  function index($service_provider, $category) {
      ...
      $data   = PhysicalTherapy::where([["insurance_company", $acount->insurance_company], ["category", $category], ["service_provider", $service_provider]])->get();
      $name   = PhysicalTherapyDepartment::where("id", $category)->first();
      return view("physical_therapy.physical_therapy", compact("data", "name"));
  }
