...
function examination($id) {
  ...
  ->select("recepion.*", "employees.name as emp_name", "companies.name as company_name", "medical_examination_booking.price", "medical_examination.name as medical_examination_name",  "category.name as category_name",  "users.name as doctor")
  ...
  return view("project.examination", compact("insurance", "normal", "title"));
}
...
//  }
    ...
    if(count($request->surgery) > 1) :
      ...
      foreach($request->surgery as $item){
        if($item > 0) :
          $element = Surgery::where("id", $item)->first();
