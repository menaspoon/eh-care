...
class MedicalExaminationApproveController extends Controller {

  function index() {
      $acount = User::where("id", Auth::id())->first();
      //$company  = Companies::where("id", $company)->first();
      $medical_examination  = MedicalExamination::where("insurance_company", $acount->insurance_company)->get();
      ...
      return view("approve.medical_examination.index", compact("medical_examination", "contract",  "approve", "service_provider"));
  }
  ...
  function details($id) {
      ...
      $ApproveServiceDetails = ApproveServiceDetails::join("medical_examination", "medical_examination.id", "=", "approve_service_details.service")
      ->where("type", "medical_examination")->where("approve_id", $approve->id)->get();
