...
class MedicalExaminationController extends Controller {
  ...
  function index($service_provider, $category) {
      ...
      $data   = MedicalExamination::where([["insurance_company", $acount->insurance_company], ["category", $category], ["service_provider", $service_provider]])->get();
      $name   = Category::where("id", $category)->first();
      return view("project.medical_examination", compact("data", "name"));
  }
